...
sub handle_exif {
	...
	my ($type, $seq, $ext) = ($1, $2, $3);
	my $new = $el->xDateTime;
	if ( $new && $new =~ /(\d\d\d\d):(\d\d):(\d\d) (\d\d):(\d\d):(\d\d)/ && $2 != 0 ) {
	    $new = "$1$2$3$4$5$6$seq";
	}
	else {
	    ...
	    $new = sprintf("%04d%02d%02d%02d%02d%02d$seq",
...
sub xDateTime {
    my ( $self ) = @_;
    my $skew = $skew;
    my $dt = $self->DateTimeOriginal || $self->CreateDate || $self->MediaCreateDate || $self->DateTime;

    if ( $self->TimeZone ) {
	my ( $m, $h, $min ) = $self->TimeZone =~ /^([-+])(\d+):(\d+)/;
	$min += 60 * $h;
	$min = -$min if $m eq "-";
	$skew += $min * 60;
    }
    return ($dt) unless $skew;

    my $fd = $dt || "";
    if ( $fd =~ /(\d\d\d\d):(\d\d):(\d\d) (\d\d):(\d\d):(\d\d)/ && $2 != 0) {
	use Time::Local;
	my $time = timelocal($6,$5,$4,$3,$2-1,$1)+$skew;
	my @t = localtime($time);
